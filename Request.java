...
public class Request {
	...
	public byte[] getRequest(){
		...
		ByteBuffer req = ByteBuffer.allocate(12 + 5 + qNameLength);
		req.put(createRequestHeader());
		req.put(createQuestionHeader(qNameLength));
        return req.array();
	}
	...
	private byte[] createQuestionHeader(int qNameLength){
		...
		//Add Query Type
		question.put(hexStringToByteArray("000" + hexValueFromQtype(type)));
		question.put((byte) 0x00);
	...
	private char hexValueFromQtype(Qtype type){
		if (type == Qtype.A) {
			return '1';
